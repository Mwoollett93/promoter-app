...
function parseTier(raw: unknown, ticketInventory: number): WizardFinanceTierV1 | null {
  if (!raw || typeof raw !== "object") return null;
  const value = raw as Record<string, unknown>;
  const allocationQty =
    isFiniteNumber(value.allocationQty)
      ? value.allocationQty
      : isFiniteNumber(value.allocationPct)
        ? (ticketInventory * value.allocationPct) / 100
        : Number.NaN;
  if (
  ...
  return {
    ...
    allocationQty: Math.max(0, Math.round(allocationQty)),
...
export function loadWizardFinanceDraft(): WizardFinanceDraftV1 | null {
  ...
  try {
    ...
    const ticketInventory = Math.max(0, Math.round(parsed.ticketInventory));
    const costs = parsed.costs.map(parseCostItem);
    const tiers = parsed.tiers.map((tier) => parseTier(tier, ticketInventory));
  ...
  const payload: WizardFinanceDraftV1 = {
    ...
    tiers: draft.tiers
      ...
      .map((item) => ({
        ...
        allocationQty: Math.max(0, Math.round(item.allocationQty)),
...
export function clearWizardFinanceDraft(): void {
  if (typeof window === "undefined") return;
  try {
